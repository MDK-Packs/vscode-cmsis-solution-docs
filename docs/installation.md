...
| Generate Clang Setup | Create or update the project [`.clangd` configuration](./userinterface.md#configure-intellisense-with-clangd) for the active solution context. The configuration uses the compilation database and compiler macro headers generated by `cbuild setup`. |
